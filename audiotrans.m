...
conf.interpolator_type = 'cubic'; % 'linear', 'cubic', 'none'

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% test for different f_s
% conf.f_s     = 2100;   % sampling rate  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

conf.f_s     = 20000;   % sampling rate  
...
for k=1:conf.nframes
    ...
    rawtxsignal = [zeros(size(rawtxsignal)), rawtxsignal]; 
...
%     wavwrite(rawtxsignal,conf.f_s,16,'out.wav')   
    ...
    elseif strcmp(conf.audiosystem,'bypass')
        rawrxsignal = rawtxsignal(:,2);
        rxsignal    = rawrxsignal;
    end
    
    % Plot received signal for debgging
    figure(2);
    plot(rawtxsignal(:, 2)); hold on;
    ...
    [rxbits conf]       = rxtest(rxsignal, normtxsignal,conf, k);
    
    res.rxnbits(k)      = length(rxbits);  
    res.biterrors(k)    = sum(rxbits ~= txbits);
    res.bers(k)         = mean(rxbits ~= txbits);
...
figure(6); semilogy(distances, res.bers, '-x', 'Linewidth', 2); 
